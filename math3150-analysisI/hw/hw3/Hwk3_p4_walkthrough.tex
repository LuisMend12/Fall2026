...
\hrule
\bigskip
\noindent\textbf{\large What's left for you: Steps 3 and 4.}

\paragraph{3. Cancel. $\leftarrow$ \emph{you are here}} Distribute the minus
sign, and the two $n\,S_n$ terms cancel. Write what remains:
\[
  \bigl(n\,S_n + n\,x_{n+1}\bigr) - \bigl(n\,S_n + S_n\bigr)
  = \blank \;-\; \blank .
\]
...
\paragraph{4. Show the numerator is $\ge 0$. $\leftarrow$ \emph{then this}} This is where
``increasing'' is used.
\begin{itemize}
...
\paragraph{5. Conclude.} Numerator $\ge 0$ and denominator $>0$, so
$y_{n+1}-y_n\ge 0$.
