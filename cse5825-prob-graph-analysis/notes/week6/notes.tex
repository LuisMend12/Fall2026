...
\subsubsection{Example: (Restricted) Boltzmann machine}

Visible units $\mathbf{v}$ and hidden units $\mathbf{h}$ with energy
$E(\mathbf{v},\mathbf{h}) = -\mathbf{v}^\top W \mathbf{h} - \mathbf{b}^\top
\mathbf{h} - \mathbf{c}^\top \mathbf{v}$ and
$P(\mathbf{v},\mathbf{h}) = \frac{1}{Z}\exp(-E(\mathbf{v},\mathbf{h}))$; in
the \emph{restricted} version there are no visible-visible or
hidden-hidden edges, only a bipartite visible-hidden structure.
